export const getExpandPrompt = (simpleData: {
  brand: {
    name: string
  ...
  typography: {
    heading: { name: string; reason?: string }
    body: { name: string; reason?: string }
    mono: { name: string; reason?: string }
  }
}) => `
...
14. fontReasons: 폰트 선택 이유 — heading/body/mono 각각 왜 이 폰트를 골랐는지 (각 1문장)
    - 브랜드 성격·가독성·분위기와 연결해서 설명할 것
    - heading: 제목용 폰트를 고른 이유
    - body: 본문용 폰트를 고른 이유
    - mono: 코드·숫자용 폰트를 고른 이유
15. Typography Scale:
    - display: size 56, weight 700
    - h1: size 40, weight 700
    - h2: size 32, weight 600
    - h3: size 24, weight 600
    - h4: size 20, weight 500
    - bodyL: size 18, weight 400
    - bodyM: size 16, weight 400
    - bodyS: size 14, weight 400
...
=== 출력 JSON ===
{
  ...
  "typography": {
    ...
    "fontReasons": {
      "heading": "제목 폰트를 고른 이유 (한 문장)",
      "body": "본문 폰트를 고른 이유 (한 문장)",
      "mono": "모노 폰트를 고른 이유 (한 문장)"
    },
    "scale": {
